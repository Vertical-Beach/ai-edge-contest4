...
uint8_t colorB[] = {142, 0, 0, 255, 0};
uint8_t colorG[] = {47, 0, 255, 0, 0};
uint8_t colorR[] = {69, 255, 255, 0, 0};
...
void runSegmentation(DPUTask *task, bool &is_running) {
    ...
    while (is_running) {
        ...
        // assume input and output resolution is same
        // resize(segMat, showMat, Size(inWidth, inHeight), 0, 0, INTER_NEAREST);
        Mat resized_img(outHeight, outWidth, CV_8UC3);
        resize(img, resized_img, Size(outWidth, outHeight), 0, 0, INTER_NEAREST);
        for (int i = 0; i < segMat.rows * segMat.cols * 3; i++) {
            resized_img.data[i] = resized_img.data[i] * 0.45 + segMat.data[i] * 0.55;
        }

        // Put image into display queue
        mtx_display_queue.lock();
        display_queue.push(make_pair(index, resized_img));
